<x-layouts>
    <section class="px-6 py-8 ">
        ...
        <div class="mb-6 mx-auto rounded-xl p-8 max-w-xl bg-gray-100 border border-gray-200">
            <h1 class="text-blue-500 font-bold text-sm text-center mb-2">EDIT & PUBLISH POST</h1>
            <h1 class="text-gray-500 font-bold text-xl text-center mb-7">{{ $post->title }}</h1>
            <form action="/admin/post/{{ $post->id }}" method="post" enctype="multipart/form-data">
                ...
                <div class="mb-6 border border-gray-400 p-2 rounded-xl">
                    ...
                    >
                    <img src="/storage/{{ $post->thumbnail }}" class="rounded-xl w-20 h-20">
                    </div>
                </div>
                {{--                excertp--}}
                <div class="mb-6">
                    ...
                    <textarea name="excertp" id="excertp" rows="3"
                              class="w-full border border-gray-200 text-sm focus:outline-none focus:ring
                                      rounded-xl p-4 ">{{ $post->excertp  }}</textarea>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="body" id="body" rows="5"
                              class="w-full border-2 border-gray-200 text-sm focus:outline-none focus:ring
                                      rounded-xl p-4 ">{{ $post->body  }}</textarea>
